  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
      <header className="mb-10 text-center">
        <h1 className="text-3xl sm:text-4xl font-bold mb-2">Emergency Alerts</h1>
      ...
      <div className="space-y-6 sm:space-y-8">
        {alerts.length > 0 ? alerts.map((alert, idx) => (
          <motion.div 
            ...
            className={`p-6 sm:p-8 rounded-[2rem] border shadow-xl hover:translate-y-[-4px] transition-all duration-300 ${
            ...
            <div className="flex flex-col md:flex-row items-start gap-4 sm:gap-6">
              ...
              <div className="flex-1 space-y-4 w-full">
                <div className="flex flex-wrap justify-between items-center gap-2">
                  <h3 className="text-xl sm:text-2xl font-bold text-white tracking-tight">{alert.village}</h3>
                  <span className="text-[10px] sm:text-xs font-mono text-slate-500 bg-slate-800/50 px-3 py-1 rounded-full border border-white/5">
          ...
          <div className="text-center py-16 sm:py-24 bg-slate-900 rounded-[2.5rem] border border-dashed border-white/10 px-4">
            <Info className="w-12 h-12 sm:w-16 sm:h-16 text-slate-700 mx-auto mb-6" />
            <p className="text-slate-500 text-lg sm:text-xl font-medium italic">No emergency alerts have been broadcasted yet.</p>
          </div>
        )}
      </div>
    </div>
